...
struct SortPanelView: View {
    ...
    public func _performSorting(kind: Sorting) {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: kind.sortingSpeed, repeats: true) { timer in
            switch kind {
            case .bubble: do {
                self.barList[bbs_select].moving = false
                //self.barList[bbs_select + 1].moving = false
                self._bubbleSort()
            }
    ...
    @State private var bbs_select = 0;
        ...
        for i in 0..<length - 1 {
            for j in 0..<length - i - 1 {
                ...
                if self.barList[j].value > self.barList[j + 1].value {
                    bbs_select = j
                    //self.barList[j].moving = true
                    self.barList[j+1].moving = true
